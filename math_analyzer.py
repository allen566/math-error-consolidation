...
第三步：根据题目要求分析导数
- 若求单调区间：解 f'(x) > 0（递增）或 f'(x) < 0（递减）
- 若求极值：找 f'(x) = 0 的点，判断两侧符号变化
- 若求参数范围：根据单调性或极值条件建立不等式

第四步：参数分类讨论（如需要）
对于含参数的函数，可能需要对参数进行分类讨论
...
第二步：确定运算类型
根据题目要求，确定是求数量积、模长、夹角，还是判断垂直/平行
...
第一步：确定几何体类型
判断是正方体、长方体、圆柱、圆锥，还是其他类型

第二步：明确已知条件
从题目中提取边长、半径、高、棱长等已知条件

第三步：选择相应公式
根据几何体类型和题目要求，选择相应的公式
